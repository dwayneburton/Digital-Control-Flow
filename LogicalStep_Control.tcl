...
if {[is_project_open]} {
	if {[string compare $quartus(project) "LogicalStep_Control"]} {
		puts "Project LogicalStep_Control is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists LogicalStep_Control]} {
		project_open -revision LogicalStep_Control_top LogicalStep_Control
	} else {
		project_new -revision LogicalStep_Control_top LogicalStep_Control
	}
...
#	set_global_assignment -name PROJECT_OUTPUT_DIRECTORY output_files
	set_global_assignment -name SOURCE_FILE LogicalStep_Control_top.vhd
	...
	set_global_assignment -name TCL_SCRIPT_FILE LogicalStep_Control.tcl
